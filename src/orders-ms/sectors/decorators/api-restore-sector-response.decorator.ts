...
export const ApiRestoreSectorResponse = () =>
  applyDecorators(
    ApiBearerAuth('jwt'),
    ApiHeader({
    ...
    ApiOperation({ summary: 'Restore a soft-deleted sector (staff only)' }),
    ApiResponse({
      status: 200,
      description: 'Sector restored',
      schema: {
        type: 'object',
        example: {
    ...
    ApiBadRequestResponse({
      description: 'Sector is not deleted',
      schema: { example: { message: 'Sector is not deleted', statusCode: 400 } },
    }),
    ApiNotFoundResponse({
